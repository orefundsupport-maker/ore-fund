...
// リポジトリ内(例: app/api/og/fund/fonts/NotoSansJP-Bold.otf)にバンドルしたフォントを
// import.meta.url経由でfetchする。外部CDNへのネットワークI/Oが発生しないため、
// 初回生成の速度も安定し、フォント配布元の障害にも影響されない。
async function loadJapaneseFont() {
  const fontUrl = new URL('./fonts/NotoSansJP-Bold.otf', import.meta.url);
  const res = await fetch(fontUrl);
  return res.arrayBuffer();
}

function fallbackImage() {
  // クロール失敗時に真っ白/500エラーを出すより、最低限のブランド画像を返す
